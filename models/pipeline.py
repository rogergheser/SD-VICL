...
from models.ldm import LatentProcessor, UNetWrapper
...
class DiffusionPipeline:
    ...
    @torch.no_grad()
    def generate(self, samples: list[Image.Image], config: GenerationConfig, callback: Callable | None = None) -> list[Image.Image]:
        """
        ...
        latents = latents + [latents[0].detach().clone()]
        ...
        for i, t in enumerate(self.scheduler.timesteps):
            ...
            reweighting_factor = config.swap_guidance * (config.num_inference_steps - i) / config.num_inference_steps
